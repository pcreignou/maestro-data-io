...
export const authorizeCRM = (req: IReqQuery<AuthorizeQuery>, res: IRes) => {
  const {
    query: { redirect_uri: redirectUri, state },
  } = req;
  return res.render('index.pug', {
...
export const generateCRMAuthToken = (req: IReq<GenerateAuthTokenBody>, res: IRes) => {
 const accessToken = jwt.sign({ type: 'access_token', sub: crypto.randomUUID(), email: `${crypto.randomUUID()}@test.com` }, env.JWT_SECRET_KEY, {
    expiresIn: 3600,
  });
  const refreshToken = jwt.sign({ type: 'refresh_token' }, env.JWT_SECRET_KEY);
...
export const getCRMUserInfo = (req: IReq, res: IRes) => {
  const token = req.headers.authorization?.split(' ')[1];
  if (!token) {
    throw new Error();
  }
